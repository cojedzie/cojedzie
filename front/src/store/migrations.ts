...
const migrations: Migration<any, any>[] = [
    {
        ...
        key: "vuex",
        skip: state =>
            !state || !state.favourites || !state.favourites.favourites || state.favourites.favourites.length == 0,
        up: async state => {
            const current = state.favourites.favourites;

            const ids = current.flatMap(favourite => favourite.state.stops).filter(distinct);
            const stops = await api.get("v1_stop_list", {
                query: { id: ids },
                version: "^1.0",
            });

            const lookup = stops.data.reduce((lookup, stop) => ({ ...lookup, [stop.id]: stop }), {});
