...
function reportProgress() {
    ...
    const percentile80th = map(requestTimeSorted, v => quantile(v, 0.8));
    const percentile50th = map(requestTimeSorted, v => quantile(v, 0.5));
    const percentile95th = map(requestTimeSorted, v => quantile(v, 0.95));
    ...
    argv.verbose &&
        console.log(
            `Avg: ${Math.round(mean(allTimes))}; Err: ${errorCount}; Fail: ${failCount}; percentiles: 50th - ${quantile(
                allTimes,
                0.5
            )}, 80th - ${quantile(allTimes, 0.8)}, 95th - ${quantile(allTimes, 0.95)}`
        );
    ...
    totalRequestTimes = merge(
        totalRequestTimes,
        map(requestTimeAvgs, v => [v]),
        (tot, cur) => [...(tot || []), ...(cur || [])]
    );
